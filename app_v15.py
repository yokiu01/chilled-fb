# 춤마루 MVP v15 Streamlit (2025.12.23)
# v15: streamlit cloud 배포용 코드 교체(Google sheets DB 연동)
...
def read_sheet(worksheet_name):
    """구글 시트에서 데이터를 읽어와 딕셔너리로 변환"""
    try:
        # ttl=0 설정으로 실시간 데이터를 가져옵니다.
        df = conn.read(worksheet=worksheet_name, ttl=0)
        # 첫 번째 열을 키(ID)로 하는 딕셔너리 변환
        return df.dropna(how="all").set_index(df.columns[0]).to_dict('index')
    except Exception:
        return {}

def update_sheet(worksheet_name, data_dict):
    """딕셔너리 데이터를 구글 시트에 업데이트"""
    if not data_dict: return
    df = pd.DataFrame.from_dict(data_dict, orient='index').reset_index()
    conn.update(worksheet=worksheet_name, data=df)

# ====================3단계 테스트 순서에 맞춘 데이터 함수 ====================
# [1단계: 전문가용]
def get_experts(): return read_sheet("experts")
def save_expert(ex_id, ex_data):
    data = get_experts()
    data[ex_id] = ex_data
    update_sheet("experts", data)

# [2단계: 일반 유저/학생용]
def get_students(): return read_sheet("students")
def save_student(st_id, st_data):
    data = get_students()
    data[st_id] = st_data
    update_sheet("students", data)

# [3단계: B2B 단체용]
def get_organizations(): return read_sheet("organizations")
def save_organization(org_id, org_data):
    data = get_organizations()
    data[org_id] = org_data
    update_sheet("organizations", data)

# [공통: 진행 상황 기록]
def get_progress(): return read_sheet("progress")
def save_progress(p_id, p_data):
    data = get_progress()
    data[p_id] = p_data
    update_sheet("progress", data)
# ==================== B2B 데이터 관리 함수 ====================
    ...
    with col2:
        if st.button(t('share_guide')):
            if meme_type == "정적 이미지 (PNG)":
                ...
                st.info(
                    "💡 GIF는 인스타그램 스토리, 페이스북, 트위터에서 자동으로 재생됩니다!\n\n추천 플랫폼: 인스타그램 스토리, 트위터, 텔레그램\n\n추천 해시태그: #춤마루 #K_DNA각성 #한국무용")
...
def show_org_signup_page():
    ...
    with col2:
        ...
        selected_plan = st.selectbox("플랜 선택", plan_options, format_func=lambda
            x: f"{SUBSCRIPTION_PLANS[x]['name']} - 월 {SUBSCRIPTION_PLANS[x]['price']:,}원")
...
def show_org_dashboard_page():
    ...
    plan = SUBSCRIPTION_PLANS.get(org_sub.get('plan', 'basic'), SUBSCRIPTION_PLANS['basic']) if org_sub else \
...
def show_subscription_management_page():
    ...
    new_plan = st.selectbox("새 플랜 선택", plan_options, index=current_index, format_func=lambda
        x: f"{SUBSCRIPTION_PLANS[x]['name']} - 월 {SUBSCRIPTION_PLANS[x]['price']:,}원")
...
def show_instructor_management_page():
    ...
    st.markdown(
        f"**{t('max_instructors')}:** {max_instructors if max_instructors > 0 else '무제한'} | **현재:** {len(org_instructors)}명")
...
def show_student_management_page():
    ...
    st.markdown(
        f"**{t('max_students')}:** {max_students if max_students > 0 else '무제한'} | **현재:** {len(org_students)}명")
...
def show_custom_actions_setup_page():
    ...
    if max_expanded > 0:
        ...
        selected_expanded = [expanded_options.index(opt) for opt in selected_expanded_indices if
                             opt in expanded_options]
    ...
    if max_creative > 0:
        ...
        selected_creative = [creative_options.index(opt) for opt in selected_creative_indices if
                             opt in creative_options]
