# 춤마루 MVP v14 Streamlit (2026.01.XX)
# v14: B2B Heritage & Template 시스템 - 단체별 템플릿 커스터마이징, 동작 데이터 아카이빙, A/B 실험 지원
# v13 기능 포함: B2B 시스템 - 단체/학원 계정, 구독 플랜, 강사/학생 관리, 커스텀 동작 세트
...
MOVEMENT_TEMPLATES_FILE = DATA_DIR / "movement_templates.json"
CUSTOM_MOVEMENTS_FILE = DATA_DIR / "custom_movements.json"
MOVEMENT_LOGS_FILE = DATA_DIR / "movement_logs.json"
...
# 한국무용 동작의 표준 템플릿 (필요 시 확장 가능)
DEFAULT_MOVEMENT_TEMPLATES = {
    "han_first_breath": {
        "name": "한(恨)의 첫 숨",
        "category": "기본",
        "focus": ["상체 곡선", "호흡"],
        "key_points": ["양팔 곡선", "흉곽 확장", "고개 숙임"],
        "rhythm": "4/4 느린 장단",
        "cultural_tag": ["조선 후기", "궁중무"],
        "description": "호흡을 통해 감정을 고요하게 풀어내는 인트로 시퀀스",
        "module_reference": ["상체_XY_01", "호흡_02"],
        "heritage_notes": "궁중 정재에서 시작 동작으로 활용"
    },
    "wind_flow_step": {
        "name": "바람결 흐름걸음",
        "category": "확장",
        "focus": ["하체 이동", "손끝 진동"],
        "key_points": ["발뒤꿈치-발가락 롤링", "손목 파동"],
        "rhythm": "6/8 중간 템포",
        "cultural_tag": ["풍류무", "경상도"],
        "description": "바람을 트래킹하듯 좌우 이동하며 에너지를 모으는 동작",
        "module_reference": ["보행_05", "손목_03"],
        "heritage_notes": "풍류무 중간 Section에서 자주 등장"
    },
    "celestial_spiral": {
        "name": "천상 나선",
        "category": "창작",
        "focus": ["회전", "공간 확장"],
        "key_points": ["360° 회전", "팔 확장", "시선 상승"],
        "rhythm": "Free Flow",
        "cultural_tag": ["현대 창작", "월드투어"],
        "description": "전통 선을 현대적으로 확장한 고난도 회전 시퀀스",
        "module_reference": ["회전_07", "팔_확장_04"],
        "heritage_notes": "현대무용단 협업 프로젝트에서 도입"
    }
}

def get_movement_templates():
    """로컬 JSON에서 동작 템플릿 목록을 로드합니다."""
    return load_json(MOVEMENT_TEMPLATES_FILE)

def save_movement_templates(template_data):
    """동작 템플릿 전체를 저장합니다."""
    save_json(MOVEMENT_TEMPLATES_FILE, template_data)

def get_custom_movements():
    """단체가 제출한 커스텀 동작 데이터를 로드합니다."""
    return load_json(CUSTOM_MOVEMENTS_FILE)

def save_custom_movement(movement_id, movement_data):
    """커스텀 동작 데이터를 저장합니다."""
    custom_movements = get_custom_movements()
    custom_movements[movement_id] = movement_data
    save_json(CUSTOM_MOVEMENTS_FILE, custom_movements)

def get_movement_logs():
    """전통 동작 보존을 위한 연습/공연 로그를 로드합니다."""
    return load_json(MOVEMENT_LOGS_FILE)

def save_movement_log(log_id, log_data):
    """동작 로그를 저장하여 디지털 아카이브를 구축합니다."""
    logs = get_movement_logs()
    logs[log_id] = log_data
    save_json(MOVEMENT_LOGS_FILE, logs)

def initialize_template_library():
    """
    기본 템플릿이 존재하지 않을 경우 DEFAULT_MOVEMENT_TEMPLATES를 저장합니다.
    B2B 단체에서 추가한 템플릿과 충돌하지 않도록 최초 1회만 실행됩니다.
    """
    existing = get_movement_templates()
    if not existing:
        save_movement_templates(DEFAULT_MOVEMENT_TEMPLATES)
...
TRANSLATIONS = {
    'ko': {
        ...
        'completion_rate': '완료율',
        'view_details': '상세 보기',
        'template_library': '동작 템플릿 라이브러리',
        'movement_templates': '동작 템플릿',
        'template_metadata': '템플릿 메타데이터',
        'propose_movement': '커스텀 동작 제안',
        'base_template': '참고 템플릿',
        'movement_name': '동작 이름',
        'movement_focus': '중점 포인트',
        'key_points': '핵심 포인트',
        'movement_story': '문화적 서사 / 비하인드',
        'key_changes': '조정 사항',
        'submit_for_review': '검수 요청',
        'custom_movement_proposals': '커스텀 동작 제안 목록',
        'heritage_archive': '디지털 유산 아카이브',
        'log_practice': '연습 기록 남기기',
        'event_type': '행사 유형',
        'region': '지역',
        'dancer_count': '참여 인원',
        'notes': '비고',
    ...
    'en': {
        ...
        'template_library': 'Movement Template Library',
        'movement_templates': 'Movement Templates',
        'template_metadata': 'Template Metadata',
        'propose_movement': 'Propose Custom Movement',
        'base_template': 'Base Template',
        'movement_name': 'Movement Name',
        'movement_focus': 'Focus Points',
        'key_points': 'Key Points',
        'movement_story': 'Cultural Narrative / Story',
        'key_changes': 'Adjustments',
        'submit_for_review': 'Submit for Review',
        'custom_movement_proposals': 'Custom Movement Proposals',
        'heritage_archive': 'Digital Heritage Archive',
        'log_practice': 'Log Practice/Performance',
        'event_type': 'Event Type',
        'region': 'Region',
        'dancer_count': 'Dancer Count',
        'notes': 'Notes',
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.org_logged_in:
            ...
            if st.button(t('custom_actions'), use_container_width=True):
                ...
                st.rerun()
            if st.button(t('template_library'), use_container_width=True):
                st.session_state.current_step = 'template_library'
                st.rerun()
            if st.button(t('heritage_archive'), use_container_width=True):
                st.session_state.current_step = 'heritage_archive'
                st.rerun()
    ...
    elif st.session_state.current_step == 'template_library':
        show_template_library_page()
    elif st.session_state.current_step == 'heritage_archive':
        show_heritage_archive_page()
...
def show_template_library_page():
    """동작 템플릿 라이브러리를 확인하고 커스텀 동작을 제안하는 페이지."""
    templates = get_movement_templates()
    custom_movements = get_custom_movements()
    
    st.markdown(f"## {t('template_library')}")
    
    if not templates:
        st.info("등록된 템플릿이 없습니다. 관리자에게 문의하세요.")
        return
    ...
    selected_template = st.selectbox(
        t('select_template'),
        template_keys,
        index=template_keys.index(st.session_state.selected_template_id) if st.session_state.selected_template_id in template_keys else 0,
        format_func=lambda x: templates[x]['name']
    )
    st.session_state.selected_template_id = selected_template
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(f"- **{t('movement_focus')}:** {', '.join(template_data['focus'])}")
        st.markdown(f"- **{t('key_points') if 'key_points' in template_data else 'Key Points'}:** {', '.join(template_data['key_points'])}")
        st.markdown(f"- **{t('module_reference')}:** {', '.join(template_data['module_reference'])}")
    with col2:
        st.markdown(f"- **리듬:** {template_data['rhythm']}")
        st.markdown(f"- **{t('cultural_tag')}:** {', '.join(template_data['cultural_tag'])}")
        st.markdown(f"- **{t('notes')}:** {template_data['heritage_notes']}")
    
    st.markdown(f"**{t('movement_story')}:** {template_data['description']}")
    
    # 커스텀 동작 제안 폼
    st.markdown("---")
    st.markdown(f"### {t('propose_movement')}")
    
    if not st.session_state.org_logged_in:
        st.info("단체 로그인이 필요합니다.")
    else:
        with st.form("custom_movement_form"):
            movement_name = st.text_input(t('movement_name'))
            focus_points = st.text_input(t('movement_focus'), placeholder="예: 어깨 곡선, 빠른 이동")
            cultural_story = st.text_area(t('movement_story'))
            adjustments = st.text_area(t('key_changes'), placeholder="예: 팔 각도 +10°, 리듬 BPM +5")
            submit = st.form_submit_button(t('submit_for_review'), type="primary")
            
            if submit:
                if movement_name:
                    movement_id = f"custom_{int(time.time())}"
                    movement_payload = {
                        'id': movement_id,
                        'org_id': st.session_state.org_id,
                        ...
                        'name': movement_name,
                        'focus': focus_points,
                        'story': cultural_story,
                        'adjustments': adjustments,
                        'status': 'submitted',
                        'created_at': datetime.now().isoformat()
                    }
                    save_custom_movement(movement_id, movement_payload)
                    st.success(t('data_saved'))
                    st.rerun()
                else:
                    st.error("동작 이름을 입력해주세요.")
    
    # 제안 목록
    st.markdown("---")
    st.markdown(f"### {t('custom_movement_proposals')}")
    if custom_movements:
        for movement in custom_movements.values():
            with st.expander(f"{movement['name']} · {movement['status']}"):
                st.markdown(f"- **{t('base_template')}:** {templates.get(movement['base_template'], {}).get('name', movement['base_template'])}")
                st.markdown(f"- **{t('movement_focus')}:** {movement.get('focus', '')}")
                st.markdown(f"- **{t('movement_story')}:** {movement.get('story', '')}")
                st.markdown(f"- **{t('key_changes')}:** {movement.get('adjustments', '')}")
                st.markdown(f"- **Status:** {movement['status']}")
                if st.session_state.org_logged_in and movement.get('org_id') == st.session_state.org_id:
                    col_a, col_b = st.columns(2)
                    with col_a:
                        if st.button("승인 처리", key=f"approve_{movement['id']}"):
                            movement['status'] = 'approved'
                            save_custom_movement(movement['id'], movement)
                            st.success("승인 상태로 변경되었습니다.")
                            st.rerun()
                    with col_b:
                        if st.button("반려 처리", key=f"reject_{movement['id']}"):
...
def show_heritage_archive_page():
    """디지털 유산 보존을 위한 동작 로그 페이지."""
    st.markdown(f"## {t('heritage_archive')}")
    
    templates = get_movement_templates()
    custom_movements = get_custom_movements()
    movement_logs = get_movement_logs()
    
    # 로그 입력 폼 (로그인 여부와 무관하게 접근 가능하도록 설계)
    st.markdown(f"### {t('log_practice')}")
    movement_options = []
    for template_id, data in templates.items():
        movement_options.append((template_id, data['name'], 'template'))
    for movement in custom_movements.values():
        if movement.get('status') == 'approved':
            movement_options.append((movement['id'], movement['name'], 'custom'))
    
    if not movement_options:
        st.info("사용 가능한 동작이 없습니다. 템플릿이나 승인된 커스텀 동작을 먼저 등록해주세요.")
        return
    
    with st.form("heritage_log_form"):
        selected_option = st.selectbox(
            "동작 선택",
            movement_options,
            format_func=lambda x: f"{x[1]} ({'템플릿' if x[2]=='template' else '커스텀'})"
        )
        event_type = st.text_input(t('event_type'), placeholder="예: 정기 공연, 클래스")
        region = st.text_input(t('region'), placeholder="예: 서울, 부산, 뉴욕")
        dancer_count = st.number_input(t('dancer_count'), min_value=1, value=5)
        notes = st.text_area(t('notes'))
        submitted = st.form_submit_button(t('submit_for_review'))
        
        if submitted:
            log_id = f"log_{int(time.time())}"
            log_payload = {
                'id': log_id,
                'movement_id': selected_option[0],
                'movement_name': selected_option[1],
                'movement_type': selected_option[2],
                'event_type': event_type,
                'region': region,
                'dancer_count': dancer_count,
                'notes': notes,
                'timestamp': datetime.now().isoformat(),
                'org_id': st.session_state.org_id if st.session_state.org_logged_in else None
            }
            save_movement_log(log_id, log_payload)
            st.success(t('data_saved'))
            st.rerun()
    
    # 로그 표시
    st.markdown("---")
    st.markdown(f"### {t('movement_logs')}")
    if movement_logs:
        df = pd.DataFrame(movement_logs.values())
