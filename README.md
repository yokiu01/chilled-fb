...
> **최신 버전**: v16.0 (2025.12.29)
> **권장 실행 파일**: `app_v16.py`
> **신규 기능**: 실시간 자세 감지 테스트 (MediaPipe Pose)
...
### 🎯 v16.0 (2025.12.29) - 실시간 자세 감지 테스트 시스템
**파일**: `app_v16.py`

#### 신규 기능
- **실시간 자세 감지 테스트 페이지**
  - MediaPipe Pose를 활용한 33개 랜드마크 실시간 감지
  - 웹캠 기반 실시간 스트리밍
  - FPS (Frames Per Second) 실시간 표시
  - 자세 감지 상태 시각적 피드백

- **고급 설정 옵션**
  - 감지 신뢰도 임계값 조절 슬라이더 (0.0-1.0)
  - 추적 신뢰도 임계값 조절 슬라이더 (0.0-1.0)
  - 랜드마크 표시 ON/OFF 토글
  - 웹캠 해상도 선택 (640x480, 800x600, 1280x720)

- **랜드마크 데이터 저장 및 내보내기**
  - 실시간 랜드마크 데이터 기록 기능
  - CSV 형식 내보내기 (Bi-LSTM 학습용 데이터)
  - JSON 형식 내보내기 (구조화된 데이터)
  - 프레임별 타임스탬프 및 좌표 (x, y, z, visibility) 저장
  - 기록된 프레임 수 실시간 표시

- **사용자 인터페이스 개선**
  - 웹캠 시작/중지 버튼
  - MediaPipe Pose 랜드마크 정보 가이드 (33개 포인트 설명)
  - 최근 감지된 랜드마크 좌표 실시간 표시
  - 상세 사용 가이드 및 팁 제공

- **시스템 최적화**
  - 로그인/비밀번호 인증 시스템 제거 (즉시 접근 가능)
  - 메모리 효율적인 프레임 처리
  - 리소스 자동 정리 및 관리
  - 에러 핸들링 강화 (웹캠 연결 오류, MediaPipe 초기화 실패 등)

#### 기술적 구현
- `show_pose_test_page()`: 실시간 자세 감지 메인 함수
- `convert_landmarks_to_csv()`: 랜드마크 데이터를 CSV 형식으로 변환
- MediaPipe Pose 통합 (model_complexity=1)
- OpenCV 웹캠 스트리밍 최적화
...
### 2. Python 가상환경 설정 (선택사항, 권장)

#### Windows (Miniconda)
```bash
conda create -n choomaru python=3.11
conda activate choomaru
```
...
### 4. 애플리케이션 실행
```bash
# 최신 버전 실행 (권장) - 실시간 자세 감지 포함
streamlit run app_v16.py

# 또는 특정 버전 실행
streamlit run app_v11.py  # DNA 갤러리 포함
streamlit run app_v8.py   # GIF 밈 생성
...
### 🆕 실시간 자세 감지 테스트 (v16.0 신규)

1. **접속**: 사이드바에서 "📹 실시간 자세 감지" 버튼 클릭
2. **설정 조정** (선택사항):
   - 감지 신뢰도 임계값 조절 (기본값: 0.5)
   - 추적 신뢰도 임계값 조절 (기본값: 0.5)
   - 웹캠 해상도 선택 (기본값: 640x480)
   - 랜드마크 표시 ON/OFF
3. **웹캠 시작**: "▶️ 웹캠 시작" 버튼 클릭
4. **자세 감지**:
   - 카메라 앞에서 2-3m 거리 유지
   - 몸 전체가 화면에 나오도록 위치 조정
   - 33개 랜드마크가 실시간으로 표시됨
   - FPS 및 감지 상태 확인
5. **데이터 저장** (선택사항):
   - 사이드바에서 "랜드마크 데이터 기록" 활성화
   - CSV 또는 JSON 형식으로 다운로드
   - Bi-LSTM 학습용 데이터로 활용 가능

### 💡 자세 감지 팁
- 조명이 밝은 곳에서 사용하세요
- 배경이 단순할수록 감지 정확도가 높아집니다
- 어두운 색 옷보다 밝은 색 옷이 감지에 유리합니다
- 전신이 나오도록 카메라 위치를 조정하세요
